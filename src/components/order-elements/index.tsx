...
export default function OrderElement({
  ...
  return (
    <div
      onClick={() => {
        navigate("/supplier/orders/" + order_id);
      }}
      className="cursor-pointer p-4 items-center justify-center border-2 border-[#0097d4] rounded-lg flex flex-row"
    >
      <div className="flex-grow">
        <div className="font-bold">
          REFNO : #{String(number ?? 0).padStart(4, "0")}
        </div>
        <div>Item : {item}</div>
          ...
          <div className="flex flex-col gap-2">
            <button
              onClick={() => {
                onChangeState &&
                  order_id &&
                  onChangeState(OrderStates.ACCEPTED, order_id);
              }}
            ...
            <button
              onClick={() => {
                onChangeState &&
                  order_id &&
                  onChangeState(OrderStates.REJECTED, order_id);
              }}
